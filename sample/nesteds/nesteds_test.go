...
func TestTree(t *testing.T) {
	...
	data := make([]*Set, 0, 100)
	dataChild := make([]*Set, 0, 100)
	// CREATE
	for i := 0; i < 100; i++ {
		obj := NewSet(env.Db, 1, 0)
		obj, err := obj.Create(ctx, "00000: "+strconv.Itoa(i))
		if err != nil {
			t.Fatal(err)
		}
		...
		dataChild = append(dataChild, obj)
	...
	for i := range data {
		if i%5 == 0 {
			if err := data[i].Move(ctx, dataChild[i]); err != nil {
				t.Fatal(err)
			}
		}
	}
	...
	for i := range data {
		if i%5 == 0 {
			if err := data[i].Copy(ctx, dataChild[i]); err != nil {
				t.Fatal(err)
			}
		}
	}
	...
	if _, err := queries.Raw(
		`UPDATE `+table+` SET keyl = 0, keyr = 0, level = 0 WHERE tree = 1`,
