...
func (t *Tool) Check(ctx context.Context) error {
	...
	reqList := []Set{}
	if err := queries.Raw(sqlCheck1).Bind(ctx, t.db, &reqList); err != nil {
		return err
	}
	if len(reqList) > 0 {
	...
	req := &Set{}
	if err := queries.Raw(sqlCheck2).Bind(ctx, t.db, req); err != nil {
		return err
	}
	if req.Keyl != 1 {
	...
	reqList = []Set{}
	if err := queries.Raw(sqlCheck3).Bind(ctx, t.db, &reqList); err != nil {
		return err
	}
	if len(reqList) > 0 {
	...
	reqList = []Set{}
	if err := queries.Raw(sqlCheck4).Bind(ctx, t.db, &reqList); err != nil {
		return err
	}
	if len(reqList) > 0 {
	...
	reqList = []Set{}
	if err := queries.Raw(sqlCheck5).Bind(ctx, t.db, &reqList); err != nil {
		return err
	}
	if len(reqList) > 0 {
...
func (t *Tool) Repair(ctx context.Context) error {
	...
	if _, err = t.repair(ctx, tx, 0, 1, 1); err != nil {
		return err
	}
