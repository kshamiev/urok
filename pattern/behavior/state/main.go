// Состояние
// Паттерн позволяет сохранять состояние объекта из списка возможных состояний
// Подходит для организации различных ресурсов
// Справочник состояний формируется методами описанными в интерфейсе
// Создаются типы реализующие состояния (на каждый метод с дублированием заглушек для остальных)
// Далее выбранное начальное состояние привязывается к объекту (свойство) и через него идет  управление состоянием
package main
