...
func testUserToManyOrders(t *testing.T) {
	...
	queries.Assign(&b.UsersID, a.ID)
	queries.Assign(&c.UsersID, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.UsersID, b.UsersID) {
			bFound = true
		}
		if queries.Equal(v.UsersID, c.UsersID) {
			cFound = true
		}
	}
...
func testUserToManyRoles(t *testing.T) {
	...
	_, err = tx.Exec("insert into `users_roles` (`users_id`, `roles_id`) values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `users_roles` (`users_id`, `roles_id`) values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpOrders(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.UsersID) {
			t.Error("foreign key was wrong value", a.ID, first.UsersID)
		}
		if !queries.Equal(a.ID, second.UsersID) {
...
func testUserToManySetOpOrders(t *testing.T) {
	...
	if !queries.IsValuerNil(b.UsersID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.UsersID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.UsersID) {
		t.Error("foreign key was wrong value", a.ID, d.UsersID)
	}
	if !queries.Equal(a.ID, e.UsersID) {
		t.Error("foreign key was wrong value", a.ID, e.UsersID)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `binary`, `Name`: `varchar`}
	_           = bytes.MinRead
)
